...
Sakolee documents its HTTP API with the native ASP.NET Core OpenAPI generator and serves an interactive reference UI with **Scalar**.
...
- `builder.Services.AddSakoleeOpenApi()` registers the OpenAPI document and a transformer that sets the title and the `Jwt` + `ApiKey` security schemes (`Api/OpenApi/OpenApiConfiguration.cs`).
