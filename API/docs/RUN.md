...
dotnet build Sakolee.sln -c Release
```
...
Other notable settings (`Sakolee.Api`): `Authentication` (JWT/RS256 + token lifetimes), `ApiKeys`, `Hangfire`, `Retry`, `ExternalSystems`, `Serilog`, `ErrorHandling`. See the [README configuration table](../README.md#configuration).
...
```bash
# bash
ASPNETCORE_ENVIRONMENT=Development dotnet run --project src/Sakolee.Api --urls http://localhost:5080
# PowerShell
$env:ASPNETCORE_ENVIRONMENT="Development"; dotnet run --project src/Sakolee.Api --urls http://localhost:5080
```
...
```bash
dotnet run --project src/Sakolee.McpServer
```
...
```bash
dotnet test Sakolee.sln              # all tests
dotnet test tests/Sakolee.UnitTests  # unit only (no DB needed)
```
...
| Triggered import stays `Created` | The **Worker isn't running** — start `Sakolee.Workers`. |
