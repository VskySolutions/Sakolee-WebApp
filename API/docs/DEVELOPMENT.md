# Sakolee — Development Guidelines

This document defines how the Sakolee codebase is organized and how to extend it consistently. Read it before adding features. It complements the [README](../README.md).
...
- One `DbContext`: `SakoleeDbContext`. Entity mappings live in `Persistence/Configurations/*` as `IEntityTypeConfiguration<T>` (applied via `ApplyConfigurationsFromAssembly`). **No data annotations on entities.**
...
dotnet ef migrations add <Name> \
  --project src/Sakolee.Infrastructure \
  --startup-project src/Sakolee.Api \
...
- `SakoleeDbContext` applies a **global query filter** keyed to `ITenantContext.TenantId`, and **stamps `TenantId` on insert** in `SaveChanges`. You normally don't write tenant filters by hand.
...
Add the key to `Sakolee.Shared.Security.Permissions` (`area.action`), include it in `All` and the relevant `For<Role>()` set(s). It is re-seeded onto system roles on the next startup. Mirror the key into the web side (`WEB/src/composables/usePermissions.js`) so the UI can gate controls. *(Phase 14 added `settings.manage` and `records.adminDelete` this way — both granted to Tenant Admin + Super Admin.)*
...
| 4 | `Infrastructure/Persistence/SakoleeDbContext.cs` | add `DbSet<Widget>`, query filter, `StampTenant` case (see the tenant checklist above) | DbSet name plural. |
...
| 14 | `tests/Sakolee.UnitTests` | `<Type>Tests.cs` (xUnit + Moq + FluentAssertions) | `<Type>Tests`; one fact per behaviour. |
...
- **Unit tests** (`tests/Sakolee.UnitTests`): xUnit + Moq + FluentAssertions. Mock all dependencies; test pure logic (validators, rule evaluator, backoff, hashing), services with mocked repos, and controllers by instantiating them with a mocked `ClaimsPrincipal` (`WithUser(...)`). `InternalsVisibleTo` exposes Infrastructure internals.
- **Integration tests** (`tests/Sakolee.IntegrationTests`): `WebApplicationFactory<Program>` against a dedicated SQL test DB; a shared collection fixture boots the app once. Cover endpoint auth/status codes, repository round-trips, and migration idempotency.
...
- Run `dotnet test Sakolee.sln` before opening a PR; keep it green.
