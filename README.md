...
|------|---------|----------------|
| **API** | `Sakolee.Api` | HTTP entry point. Authenticates requests, serves every endpoint, owns EF Core migrations and startup seeding, hosts the Hangfire dashboard. Enqueues background jobs; never runs them. |
| **Worker** | `Sakolee.Workers` | Runs the Hangfire server: email sending and the Universal Features recurring jobs. Without it, nothing queued (including email) is ever delivered. |
| **MCP Server** | `Sakolee.McpServer` | Host placeholder for a Model Context Protocol surface (AI-agent access). |
...
- **Infrastructure** — EF Core (`SakoleeDbContext`), repositories, email, Hangfire, Serilog, security, Data Protection. Implements the Application abstractions.
...
└── src/
    ├── Sakolee.Domain          # entities, enums
    ├── Sakolee.Shared          # contracts, config options, permission catalogue, role names
    ├── Sakolee.Application     # abstractions, application services, default seeds (email templates, option sets)
    ├── Sakolee.Infrastructure  # EF Core, repositories, email, Hangfire, security, logging
    ├── Sakolee.Api             # ASP.NET Core host (controllers, middleware, auth, OpenAPI, seeding)
    ├── Sakolee.Workers         # Hangfire worker
    └── Sakolee.McpServer       # console host (MCP placeholder)
...
Configuration lives in each host's `appsettings.json` (not committed — copy `appsettings.json.example` and fill in local values) and is overridable via environment variables / secrets. Key sections (`Sakolee.Api`):
...
cd API
dotnet run --project src/Sakolee.Api

# Terminal 2 — Worker (Hangfire server: executes jobs; required for email)
dotnet run --project src/Sakolee.Workers
...
All responses use the standard envelope (`ApiResponse<T>` / `ApiErrorResponse`). One controller per area under `API/src/Sakolee.Api/Controllers`:
